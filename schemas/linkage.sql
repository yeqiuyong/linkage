...
CREATE TABLE `linkage_company` (
  ...
  `email` VARCHAR(70) DEFAULT NULL,
  `home_page` VARCHAR(120) DEFAULT NULL COMMENT '网页',
...
CREATE TABLE `linkage_driver_task` (
  ...
  `car_id`  INT(11) UNSIGNED NOT NULL DEFAULT 0 COMMENT '承运商车id',
  `cargo_no` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '货柜号',
  `cargo_type` TINYINT(4) NOT NULL DEFAULT 0 COMMENT '货柜类型',
  `status` TINYINT(4) NOT NULL DEFAULT 0 COMMENT '0接单，1打单、2提柜、3送柜、4待装货、5还柜途中、6进入码头、7已卸柜、8完成',
  `is_accept` TINYINT(4) DEFAULT NULL COMMENT '司机是否接受订单。0:拒绝，1:接受',
  `reject_reason` VARCHAR(400) DEFAULT NULL COMMENT '拒绝接受任务理由',
